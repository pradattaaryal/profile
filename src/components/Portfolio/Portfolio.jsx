  ...
  return (
    <div className='h-screen w-full mt-24 md:mt-0 ' id='projects'>
      <div className='max-w-[1140px] h-screen m-auto'>
       ...
       <div className='flex   h-[50px] justify-center mb-6'>
  <button 
    className={`border-[3px] border-black p-2 border-solid font-bold text-xl rounded-l-2xl ${filter === 'All' ? 'bg-gray-200' : ''}`}
    onClick={() => setFilter('All')}
  >
    All Project
  </button>
  <button 
    className={`border-y-[3px] border-black p-2 border-solid border-r font-bold text-xl ${filter === 'Fullstack' ? 'bg-gray-200' : ''}`}
    onClick={() => setFilter('Fullstack')}
  >
    Fullstack
  </button>
    <button 
    className={`border-y-[3px] border-black p-2 border-solid  font-bold text-xl ${filter === 'Devops' ? 'bg-gray-200' : ''}`}
    onClick={() => setFilter('Devops')}
  >
    Devops
  </button>
  <button 
    className={`border-[3px] border-black p-2 border-solid font-bold text-xl rounded-r-2xl ${filter === 'Frontend' ? 'bg-gray-200' : ''}`}
